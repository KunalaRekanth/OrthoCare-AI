...
export const analyzeTeethImages = async (leftImg, frontImg, rightImg) => {
  ...
  // 1. Validate all uploaded files (check for blank or invalid images first)
  for (const file of [frontImg, leftImg, rightImg]) {
    if (file) {
      ...
      if (result) {
        ...
        if (result.status === 'INVALID_TEETH') {
          throw new Error('One of your uploaded images does not appear to contain a teeth/braces scan. Please upload a clear dental photo.');
        }
      }
    }
  }
